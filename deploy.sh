...
(
    ...
    git remote add upstream "https://$GH_TOKEN@${GH_REF}"  # may be changed
    git fetch upstream
...
docker tag galaxy_kickstart artbio/galaxy-kickstart-base:$TRAVIS_COMMIT  # $TRAVIS_COMMIT is a standard env variable
docker tag galaxy_kickstart artbio/galaxy-kickstart-base:latest
LOGIN="docker login -e=$DOCKER_EMAIL -u=$DOCKER_USERNAME -p=$DOCKER_PASSWORD"  # defined in the Travis repository settings
...
# if additional flavors, you would build them and push them there
